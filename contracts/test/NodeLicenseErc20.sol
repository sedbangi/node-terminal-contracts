...
import "@openzeppelin/contracts-upgradeable-4.8.2/utils/StringsUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable-4.8.2/utils/Base64Upgradeable.sol";
import "@openzeppelin/contracts-upgradeable-4.8.2/access/AccessControlUpgradeable.sol";
import {IERC20Upgradeable} from "@openzeppelin/contracts-upgradeable-4.8.2/token/ERC20/IERC20Upgradeable.sol";
import {SafeERC20Upgradeable} from "@openzeppelin/contracts-upgradeable-4.8.2/token/ERC20/utils/SafeERC20Upgradeable.sol";

contract NodeLicenseErc20 is ERC721EnumerableUpgradeable, AccessControlUpgradeable {
    ...
    function initialize(address _admin, address _fundsReceiver, address _tokenAddress) public initializer {
        __ERC721_init("NodeLicenseErc20", "NLE");
    ...
    /**
     * @notice Mints new NodeLicenseErc20 tokens.
     * @param _amount The amount of tokens to mint.
     * @param _promoCode The promo code.
     */
    function mint(uint256 _amount, string calldata _promoCode) public {
        ...
        require(erc20Token.allowance(msg.sender, address(this)) >= finalPrice, "Insufficient allowance");
        erc20Token.safeTransferFrom(msg.sender, fundsReceiver, finalPrice - referralReward);

        if (referralReward > 0) {
            erc20Token.safeTransferFrom(msg.sender, address(this), referralReward);
        }
    }
    ...
    function price(uint256 _amount, string calldata _promoCode) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < pricingTiers.length; i++) {
            ...
            uint256 availableInThisTier = tierSum > supply ? tierSum - supply : 0;
    ...
    function withdrawFunds() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        erc20Token.safeTransfer(fundsReceiver, amount);
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        require(ownerAddress != address(0), "ERC721Metadata: URI query for nonexistent token");
        string memory svg = string(
            abi.encodePacked(
                "<svg xmlns='http://www.w3.org/2000/svg' width='100' height='100' style='background-color:grey;'><rect width='120' height='120' fill='none' stroke='orange' stroke-width='20' transform='scale(0.1) translate(60, 60)'/><text x='5' y='25' font-size='2.8' fill='white' font-family='monospace'>License Id: ",
                _tokenId.toString(),
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Token #',
                        _tokenId.toString(),
                        '", "description": "Licence Key NFT (ETT)", "image": "data:image/svg+xml;base64,',
                        image,
